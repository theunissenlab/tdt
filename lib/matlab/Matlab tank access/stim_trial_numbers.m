...
% 
% Added warning/error message id RCM 10-25-06
%========================================================================
...
message_id = 'CellDB:TrialNumberError';
...
if std(trials)
    msg = sprintf(message_id,'Not all stimuli have the same number of repeats\n');
    if errflag
        error(message_id,msg);
    else
        warning(message_id,msg);
    end
    
    if exist('ntrials')
        msg = sprintf(message_id,'Not all stimuli have %.0f repeats\n',ntrials);
        if errflag
            error(message_id,msg);
        else
            warning(message_id,msg);
...
if ntrials == 1
    msg = sprintf(message_id,'There is only one repeat\n');
    if errflag
        error(msg);
    else
        warning(msg);
...
% set ntrials to 0.
if ntrials == 0
    msg = sprintf('CellDB:BlockDataError','There is no data in this block\n');
    if errflag
        error(msg);
    else
        warning(msg);
