...
where_expr = [pkey_name{1} ' == ' sql_str(varargin{1})];
for jj = 2:n_key_fields
    where_expr = [where_expr ' AND ',...
        pkey_name{jj} ' == ' sql_str(varargin{jj})];
end

%% Construct SQL command and retrieve data
sql_cmd = sprtintf('SELECT %s FROM %s WHERE %s;',select_expr,...
